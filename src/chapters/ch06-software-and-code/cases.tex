...
\end{align}
Dirichlet boundary conditions are supplied at the boundary using the exact solution. This problem is chosen to allow for refinement at the corners for any solver that can take advantage of adaptivity. The solution is plotted in \reffig{fig:case01-plot}.

\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case01-u-plot.png}
    \caption{Plot of the solution for Case 1 (\refsec{sub:case01}). The mesh is adapted according to the right-hand side. Each square represents a patch, which contains an $M \times M$ finite volume grid (not shown).}
    \label{fig:case01-plot}
\end{figure}
...
\begin{align}
    u(x,y) = (x + y) \tanh(x),
\end{align}
which is shown in \reffig{fig:case02-plot-u}. The inhomogeneity found in $\beta$ is representative of an inhomogeneous media through which fluid is flowing or a variable density for pressure calculations for a viscous fluid system (i.e. Navier-Stokes). In this case, $\beta$ has a rapid change at $x = 0$ that adaptive solvers can refine around, as seen in \reffig{fig:case02-plot-beta}. For the adaptive solvers, $L_{\text{min}} = 2$ and $L_{\text{max}} = 5$ and any patch directly located to the left or right of $x = 0$ is tagged for refinement, representing a refinement around a material interface.

\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case02-u-plot-no-mesh.png}
    \caption{Plot of the solution for Case 2 (\refsec{sub:case02}).}
    \label{fig:case02-plot-u}
\end{figure}

\begin{figure}
    \centering
    \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case02-beta-plot.png}
    \caption{Plot of $\beta(x,y)$ for Case 2 (\refsec{sub:case02}). The mesh is adapted at the interface representing two different materials.}
    \label{fig:case02-plot-beta}
\end{figure}
...
% \begin{landscape}
    \begin{figure}
        ...
        \caption{Effective resolution vs. timing plots for Case 2 (\refsec{sub:case02}). The effective resolution is the number of cells per side if the mesh were refined to $L_{\text{max}}$. Each color indicates a different method/solver. Patterns indicate different stages or preconditioners. Lower is better.}
...
\end{align}
where $Y_0$ is the Bessel function of the second kind. These are plotted in \reffig{fig:case03-plots}. The Dirichlet boundary conditions are prescribed using the exact solution. As the solution is highly oscillatory throughout the domain, no adaptivity is used in this case. Rather, this case study is used to compare the performance of direct and iterative solvers for high wave number problems.

\begin{figure}
    \centering
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case03-u-plot-lambda-1-no-mesh.png}
        \caption{$\lambda = 1$}
    \end{subfigure}
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case03-u-plot-lambda-10-no-mesh.png}
        \caption{$\lambda = 10$}
    \end{subfigure}
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case03-u-plot-lambda-100-no-mesh.png}
        \caption{$\lambda = 100$}
    \end{subfigure}
    \begin{subfigure}[t]{0.48\textwidth}
        \includegraphics[width=1.0\textwidth, clip=true, trim={45 80 70 180}]{figures/case03-u-plot-lambda-1000-no-mesh.png}
        \caption{$\lambda = 1000$}
    \end{subfigure}
    \caption{Plots of the solution for Case 3 (\refsec{sub:case03}).}
    \label{fig:case03-plots}
\end{figure}
...
\begin{figure}
    ...
    \caption{Work-precision plots for Case 3 (\refsec{sub:case03}). Colors indicate different methods, while markers indicate the value of $\lambda$. Curves that jump to the upper-right region of the plot usually indicate a failure to converge. For work-precision plots, curves that lie in the lower-left portion of the plot are better.}

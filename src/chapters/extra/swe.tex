...
%
% In \citep{lannes2015new} Lannes et al. use von Neumann stability analysis to develop expressions for $\alpha$ for this model and for the classic Serre-Green-Naghdi model. They show that their new model is also stable for certain criteria for $\alpha$, namely $\alpha \ge 1$. For their modeling in \citep{lannes2015new}, they choose $\alpha = 1.159$.
%
...
% \label{sec:tstsgnm}
%
% As a final step in this derivation, we motivate the solution methodology presented in the next chapter by casting the constant-diagonal SGN model into it's dimensional version and show how to split the problem into a hyperbolic problem and an elliptic one. This will put the model into a form similar to the form used by Popinet in \citep{popinet2015quadtree}, but with a slightly modified dispersion term to account for the adjustment from the modified SGN model to the constant-diagonal SGN model.
%

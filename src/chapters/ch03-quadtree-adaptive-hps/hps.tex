...
% \ignore{
% The set of solution operators $\textbf{S}^{\tau}$ are built by recursively merging a Dirichlet-to-Neumann (DtN) operator. This Dirichlet-to-Neumann operator is part of a class of operators called Poincaré-Steklov operators \citep{quarteroni1991theory}. These operators map boundary condition data from one type to another (i.e., Dirichlet-to-Neumann, Neumann-to-Dirichlet, Dirichlet-to-Robin, etc.). The DtN operators allow the domain to be divided into a collection of patches whose union forms the entire domain.
% }

% To demonstrate the contrast between our implementation and the implementation in \citep{martinsson2019fast} and \citep{gillman2014direct}, we outline the HPS method on a binary tree given in \citep{martinsson2019fast}. The HPS method is broken into two stages: the build stage that is similar to a matrix factorization and the solve stage that is similar to the application of the factorization to a right-hand side vector. There are two necessary building blocks for the build stage: the leaf level computations and the merge process. For the leaf level matrices, a solution matrix is formed that maps Dirichlet data on the boundary $\textbf{g}$ to solution data on the interior $u_{i}$:
% \begin{align}
%     \textbf{S}^{\tau} \textbf{g}^{\tau} = \textbf{u}^{\tau}_{i}
% \end{align}
% where $\tau$ denotes a patch domain. In \citep{martinsson2019fast}, $\textbf{S}^{\tau}$ is formed from the system coefficient matrix on a patch:
% \begin{align}
...
% \end{align}
% In \citep{martinsson2015hierarchical} and \citep{gillman2014direct}, the DtN matrix is computed via four linear maps 
% \begin{align}

...
The \gls{qahps} method applied to \refeqn{eq:elliptic-pde} is broken into a factorization stage (called the build stage) and an application stage (called the solve stage). For problems in which $f(x,y)$ is non-zero, an additional ``upwards'' stage is required which handles the inhomogeneous correction.
...
Prior to the build stage of the \gls{qahps} method, we require the solution to an elliptic problem \refeqn{eq:elliptic-pde} discretized on an $M \times M$ cell-centered grid as
\begin{equation}
\mathcal{A}^{\tau}[u^{\tau}(\textbf{x})] = \Bctau \gtau + \ftau
\label{eq:patch_poisson_problem}
\end{equation}
where $\Bctau$ spreads Dirichlet boundary data $\gtau \in \real^{4M}$ to the grid and \ftau is the right hand side function $f(x,y)$ from \refeqn{eq:elliptic-pde} evaluated at cell centers.  We write the solution to this problem in terms of a {\em homogeneous operator} \Lhtau and an inhomogeneous operator \Linhtau which represent efficient solvers on the uniform Cartesian patch. The solution is then given by 
\begin{equation}
...
\label{eq:linalg_one}
\end{equation}
Taking the first equation from each of the four sets of equations in \refeqn{eq:linalg_one}, we write a system of equations for the Dirichlet data to get
\begin{equation}
...
Combining Equations (\ref{eq:sys_one}) and (\ref{eq:sys_two}), we have
\begin{equation}
\begin{aligned}
\textbf{A} \gext + \textbf{B} \gint + \hext & = \vext \\
\textbf{C} \gext + \textbf{D} \gint + \Deltah & = \zeromat.
\end{aligned}
\end{equation}
...
\left[
\begin{array}{cc|c}
\textbf{A} & \textbf{B} & \vext - \hext\\
\textbf{C} & \textbf{D} & -\Deltah\\
\end{array}\right]
\;
\Longrightarrow 
\;
\left[
\begin{array}{cc|c}
\textbf{A} - \textbf{B}\textbf{D}^{-1}\textbf{C} & 0 & \vext - \hext + \textbf{B} \textbf{D}^{-1}\Deltah\\
\textbf{D}^{-1}\textbf{C}      & \textbf{I} & -\textbf{D}^{-1}\Deltah
\end{array}\right].
\label{eq:reduced_system}
\end{equation}
We express the first row of the reduced system in equation form as 
\begin{equation}
\left(\textbf{A} - \textbf{B}\textbf{D}^{-1}\textbf{C}\right) \gext = \vext - \hext + \textbf{B} \textbf{D}^{-1}\Deltah.
...
\begin{equation}
\htau \equiv -\hext + \textbf{B} \textbf{D}^{-1}\Deltah. 
\label{eq:htau-merged}
\end{equation}
With these choices of \Ttau and \htau  we recover the expression from \refeqn{eq:vtau}.

The second row of the reduced system \refeqn{eq:reduced_system}, given by 
\begin{equation}
\gint = -\textbf{D}^{-1}\textbf{C} \gext -\textbf{D}^{-1} \Deltah,
\end{equation}
...
\begin{equation}
\Stau \equiv -\textbf{D}^{-1}\textbf{C}.
\label{eq:Stau-merged}
\end{equation}
Introducing  the operator
\begin{equation}
\Xtau \equiv -\textbf{D}^{-1},
\label{eq:xtau}
\end{equation}
...
The build stage of \gls{qahps} method is described in \refalg{alg:build_merge_uniform}. At the end of this build stage, we will have a single DtN operator \Ttau and single vector \htau for the root patch.  At all levels, though, we will have a solve operator \Stau and inhomogeneous vector \wtau.
    ...
    \begin{algorithmic}[0]
        \For{$\tau = 0, 1, \dots, N_P$} \Comment{Iterate over quadrants in uniform quadtree}
            ...
            \Else \Comment{$\tau$ has four child patches $i$, $i=\alpha$, $\beta$, $\gamma$, $\omega$.}
                ...
                \State Build merged inhomogeneous vector \htau using \refeqn{eq:htau}.
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[0]
        \For{$\tau = N,N-1,\hdots,0$} \Comment{Iterate over quadrants in uniform quadtree}
            \If{$\tau$ is a leaf-level patch}
                \State Solve elliptic problem \refeqn{eq:elliptic-pde}
            \Else{} \Comment{$\tau$ has four child patches $\alpha$, $\beta$, $\gamma$, $\omega$.}
                \State Use \refeqn{eq:solve_eqn} to map Dirichlet \gext on $\tau$ to Dirichlet data \gint on the interior shared child patch boundaries.
            \EndIf
        \EndFor
    \end{algorithmic}
...
The merging and splitting processes we have described so far applies only to four sibling patches occupying a coarser level quadrant. For a fully adaptive quadtree, however, not all quadrants are refined to the same level, resulting in coarse-fine interfaces (see \reffig{fig:adaptive_merge}). To accommodate for the coarse-fine interfaces, we coarsen entire patches with coarse-fine interfaces. This approach is similar to the approach used by \cite{babb2018accelerated}. The mesh is not changed as a result of this process, just the data on a patch, as detailed below.
...
\begin{figure}
    ...
    \begin{tabular}{ccc}
        ...
        \end{subfigure}
    \end{tabular}
    \caption{Example of a coarse-fine interface merge: (left) patches $4, 5, 6, \&\ 7$ will be merged and result in a coarse-fine interface, (middle) the data on patch 3 is averaged (coarsened), (right) merging $0, 1, 2, \&\ 3$ can continue as detailed in \refsec{sub:4-to-1merge}.}
...
A 1-to-4 split with a parent that has data that was coarsened during the 4-to-1 merge (for example, patch 5 from \reffig{fig:adaptive_merge}) needs to be un-coarsened. The requisite data that is un-coarsened during the solve stage is the boundary data, $\gext$ in Equation \refeqn{eq:solve_eqn}. This is done with the block diagonal interpolation matrices formed above:
\begin{align}
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[0]
        ...
        \For{$i = \alpha, \beta, \gamma, \omega$}
            ...
            \EndIf
        \EndFor
        \State Compute matrices $\textbf{A}, \textbf{B}, \textbf{C}, \textbf{D}$ according to \refeqn{eq:matrix_AB} and \refeqn{eq:matrix_CD}.
...
\begin{algorithm}[h]
    ...
    \begin{algorithmic}[0]
        ...
        \EndIf
        \EndFor
        \State Compute matrices $\textbf{B}, \textbf{D}$ according to \refeqn{eq:matrix_AB} and \refeqn{eq:matrix_CD}
        \State Compute $\Delta \textbf{h}$ according to \refeqn{eq:h_diff}
